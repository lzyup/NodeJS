...
//.get代表请求method是get,所以这里可以用post、delete等。这个能力很适合用于创建rest服务
app.get('/favicon.ico', function (request, response) {
    //一句 status(200) 代替了writeHead(200);end()
    response.status(200)
    return;
})
...
app.get('/game', function (request, response, next) {
    ...
    function (request, response) {
        ...
        //因为playerWon不是在中间件执行流程所属的那个事件循环里赋值
        // setTimeout(() => {
        response.status(200);
        if (result == 0) {
            response.send('平局');
